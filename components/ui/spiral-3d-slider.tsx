...
function SpiralScene({
  ...
  useFrame((_state, delta) => {
    ...
    const planeWidth = Math.min(cardWidth / factor, viewport.width * 0.4);
    const planeHeight = planeWidth / cardAspectRatio;
    const spiralRadius = Math.min(radius / factor, viewport.width * 0.4);
    const gap = Math.min(verticalGap / factor, viewport.height * 0.2);
